...
export default function AdminDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<'contacts' | 'bookings' | 'testimonies' | 'orders'>('contacts');
  ...
  const filteredTestimonies = testimonies.filter(testimony => 
    testimonyFilter === 'all' || 
    (testimonyFilter === 'approved' ? testimony.approved : !testimony.approved)
  );
          ...
          {loading.orders ? (
            ...
            <div className="space-y-4">
              {filteredOrders.map((order) => (
                <div key={order.id} className={`p-4 rounded-lg shadow ${
                  ...
                  {order.note && (
                    <div className="mt-2 p-2 bg-gray-100 rounded">
                      <p className="text-sm"><span className="font-medium">Customer Note:</span> {order.note}</p>
                    </div>
                  )}
